...
# Copyright (c) 2006 - 2007 George Nistorica
# All rights reserved.
...
is( $test, 1, "Wheel::SocketFactory Error" );
...
sub spawn_pococlsmt {
    POE::Component::Client::SMTP->send(
        ...
        Server       => 'localhost',
        Port    => ( $port + 1 ),    # this should make the component to try to
...
sub smtp_send_success {
    ...
    $test = 0;
    diag("Sending email unexpectedly succeeded!");
}

sub smtp_send_failure {
    my ( $arg0, $arg1 ) = @_[ ARG0, ARG1 ];
    print "ARG0, ", Dumper($arg0), "\nARG1, ", Dumper($arg1) if $debug;
    if ( ref( $arg1->{'POE::Wheel::SocketFactory'} ) eq 'ARRAY' ) {
        $test = 1;
    }
    else {
        $test = 0;
